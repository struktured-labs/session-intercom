...
async def diagnose_session(name: str) -> dict:
    ...
    Returns a structured dict with: session row, last-received message id from
    the file inbox bridge perspective (NOT the file inbox — that's checked by
    inbox_stats in the server layer), and any unread DM/channel counts at the
    MCP cursor layer.
    """
    db = await get_connection()
    try:
        ...
        # Count unread DMs at MCP-cursor layer
        ...
        # Count unread channel messages at MCP-cursor layer
        ...
        # Latest message addressed to this session
